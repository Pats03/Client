...
const emailEmissionsData = {
  'temethon1@gmail.com': { green: 10, yellow: 20, red: 70 },
  'temethon2@gmail.com': { green: 40, yellow: 30, red: 30 },
  'temethon3@gmail.com': { green: 25, yellow: 50, red: 25 },
  // Default case for unknown emails
  default: { green: 20, yellow: 30, red: 50 },
};

const Jar = ({email}) => {
    const [emissions, setEmissions] = useState(emailEmissionsData['default']);
  useEffect(() => {
    if (email && emailEmissionsData[email]) {
      setEmissions(emailEmissionsData[email]);
    } else {
      setEmissions(emailEmissionsData['default']);
    }
  }, [email]);
